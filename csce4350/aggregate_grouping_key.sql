...
*/
    ...
    SELECT  fld_o_id_pk, fld_o_month, fld_c_id_fk, fld_a_id_fk, fld_p_id_fk, 
            fld_o_qty, fld_o_dollars, fld_o_qty*fld_o_dollars AS calc_o_tot
    FROM tbl_orders;        
...
/*        
 ...
 c_id | new_disc | p_id | price | o_id | qty  |  dollars  | calc_tot  
------+----------+------+-------+------+------+-----------+-----------
...
*/
    ...
    SELECT  fld_c_id_pk AS c_id, ROUND(fld_c_discnt/100,2) AS new_disc, fld_p_id_pk AS p_id,
            fld_p_price AS price, fld_o_id_pk AS o_id,
            fld_o_qty AS qty, fld_o_dollars AS dollars, fld_p_price*(1-fld_c_discnt/100)*fld_o_qty AS calc_tot
...
*/
        ...
        SELECT fld_p_city, COUNT(*) AS my_cnt, SUM(fld_p_quantity) AS my_sum
        FROM tbl_products
        GROUP BY fld_p_city;
...
-- part a:
    SELECT fld_o_month, MAX(fld_o_dollars) AS my_max, ROUND(AVG(fld_o_dollars::NUMERIC),2) AS my_avg
    FROM tbl_orders
    GROUP BY fld_o_month 
    ORDER BY my_avg;
...
-- Part b:
    ...
    FROM tbl_orders
    GROUP BY fld_o_month 
        HAVING AVG(fld_o_dollars::NUMERIC)>700
    ORDER BY my_avg;
    
-- Extra point (cast to MONEY)
    SELECT fld_o_month, MAX(fld_o_dollars) AS my_max, 
            ROUND(AVG(fld_o_dollars::NUMERIC),2)::MONEY AS my_avg
    FROM tbl_orders
    GROUP BY fld_o_month 
        HAVING AVG(fld_o_dollars::NUMERIC)>700
    ORDER BY my_avg;
...
/*
    "CAST(fld_o_dollars TO NUMERIC)" is also perfectly acceptable, but "fld_o_dollars::NUMERIC" is easier.
*/
